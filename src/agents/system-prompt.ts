...
function buildDeviceUiActionsSection(params: { isMinimal: boolean; availableTools: Set<string> }) {
  ...
  return [
    "## Device UI Actions",
    '- For open-ended phone-control requests such as opening apps, navigating home/back/recents/notifications/quick_settings, tapping buttons, typing text, scrolling, or reading the visible screen, prefer `nodes(action="ui_task", objective="...")` so the gateway can observe the screen and run a closed-loop task.',
