...
function buildDeviceUiActionsSection(params: { isMinimal: boolean; availableTools: Set<string> }) {
  ...
  return [
    "## Device UI Actions",
    '- For open-ended phone-control requests such as opening apps, navigating home/back/recents/notifications/quick_settings, tapping buttons, typing text, using `ime_enter` after text entry, scrolling/swiping, or reading the visible screen, prefer `nodes(action="ui_task", objective="...")` so the gateway can observe the screen and run a closed-loop task.',
    '- Use `nodes(action="apps_resolve", appQuery="...")` before `open_app` when the user names an app and you do not already have a reliable Android package id; Android also tries to resolve human app names in `open_app`, but package ids are preferred.',
    "- Use `open_uri` for bounded `http`/`https` deep links such as opening a profile or document URL on the phone; include `packageName` only when a specific installed app should handle the link.",
    ...
    "- Use `long_click_node` for Android long-press gestures such as context menus, app shortcuts, selection handles, or UI affordances that require press-and-hold.",
